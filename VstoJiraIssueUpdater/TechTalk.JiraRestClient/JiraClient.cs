    ...
    public class JiraClient<TIssueFields> : IJiraClient<TIssueFields> where TIssueFields : IssueFields, new()
    {
        ...
        public Issue<TIssueFields> CreateIssue(String projectKey, String issueType, TIssueFields issueFields)
        {
            try
            {
                ...
                if (issueFields.components != null && issueFields.components.Count > 0)
                {
                    object[] components = new object[issueFields.components.Count];
                    for (int i = 0; i < issueFields.components.Count; i++)
                    {
                        components[i] = new { name = issueFields.components[i].name };
                    }
                    issueData.Add("components", components);
                }

                //if (issueFields.priority != null &&
                //    string.IsNullOrWhiteSpace(issueFields.priority.name) == false)
                //{
                //    issueData.Add("priority", new { name = issueFields.priority.name });
                //}
                ...
                if (issueFields.fixVersions != null && issueFields.fixVersions.Count > 0)
                {
                    object[] fixVersions = new object[issueFields.fixVersions.Count];
                    for (int i = 0; i < issueFields.components.Count; i++)
                    {
                        fixVersions[i] = new { name = issueFields.fixVersions[i].name };
                    }
                    issueData.Add("fixVersions", fixVersions);
                }
        ...
        public Issue<TIssueFields> UpdateIssue(Issue<TIssueFields> issue)
        {
            try
            {
                ...
                {
                    object[] components = new object[issue.fields.components.Count];
                    for (int i = 0; i < issue.fields.components.Count; i++)
                    {
                        components[i] = new { name = issue.fields.components[i].name };
                    }
                    updateData.Add("components", components);
                }

                //if (issue.fields.priority != null)
                //{
                //    updateData.Add("priority", new { name = issue.fields.priority.name });
                //}

                //if (issue.fields.assignee != null)
                //{
                //    updateData.Add("assignee", new { name = issue.fields.assignee.name });
                //}

                if (issue.fields.fixVersions != null && issue.fields.fixVersions.Count > 0)
                {
                    object[] fixVersions = new object[issue.fields.fixVersions.Count];
                    for (int i = 0; i < issue.fields.components.Count; i++)
                    {
                        fixVersions[i] = new { name = issue.fields.fixVersions[i].name };
                    }
                    updateData.Add("fixVersions", fixVersions);
                }
        ...
        public Project GetProject(string projectKey)
        {
            try
            {
                ...
                return deserializer.Deserialize<Project>(response);
            }
            catch (Exception ex)
            {
